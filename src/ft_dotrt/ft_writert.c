...
/*   Updated: 2023/12/30 21:29:04 by sanghupa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_dotrt(t_dotrt *rt)
{
	...
	// rt->sp[0]->color = (t_vec3){255/255, 0/255, 0/255};
	// rt->cnt_sp++;
	...
	// rt->pl[0]->color = (t_vec3){0/255, 0/255, 255/255};
	// rt->cnt_pl++;
	// rt->pl[1] = init_subrt();
	// rt->pl[1]->type = PLANE;
	// rt->pl[1]->point = (t_vec3){0.0, -10.0, 0.0};
	// rt->pl[1]->vector = (t_vec3){0, 1, 0};
	// // rt->pl[1]->color = (t_vec3){155/255, 155/255, 155/255};
	// rt->pl[1]->color = init_vector(0.48, 0.83, 0.53);
	// rt->cnt_pl++;

	// rt->cy[0] = init_subrt();
	// rt->cy[0]->type = CYLINDER;
	// rt->cy[0]->point = (t_vec3){-20, 10, 5};
	// rt->cy[0]->vector = (t_vec3){0, 1, 0};
	// rt->cy[0]->value1 = 10 / 2;	// diameter
	// rt->cy[0]->value2 = 40;	// height
	// rt->cy[0]->color = (t_vec3){0/255, 255/255, 0/255};
	// rt->cnt_cy++;
